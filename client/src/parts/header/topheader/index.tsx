...
import ChanelOptions from "../../../components/chaneloptions";
...
const TopHeader = ({ searchQuery }: TopHeaderProps) => {
  ...
  const onSearch = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!searchQuery) {
      ...
      const queryUrl = "/results?search_query=" + searchQuery;
      navigation(queryUrl);
    }
  };
  return (
    <div className="w-full flex justify-between items-center  px-2 md:px-6 text-gray-200 py-3 lg:py-1 border-b border-lightgray_323232">
      <div className="flex-1 flex justify-start items-center space-x-4 text-gray-300 hover:text-gray-100">
        <div>
          <button>
      ...
      <div className="hidden sm:flex flex-1  justify-center items-center space-x-4 w-full">
        ...
        >
          <div className="flex-1 bg-bg_gray_181818">
            <input
              type="text"
              name="search"
              defaultValue={searchQuery}
              className="focus:outline-none bg-transparent w-full text-xs px-2 py-1"
            />
          </div>
          <div className="h-full text-xs hover:text-gray-100 text-gray-300">
            <button className="bg-lightgray_323232 px-4 h-full">
              <i className="fas fa-search"></i>
            </button>
          </div>
        </form>
        <div className="hidden md:block text-xs hover:text-gray-100 text-gray-300">
          <button className="bg-bg_gray_181818 rounded-full h-7 w-7">
      ...
      <div className="flex-1">
        {!chanel.loading ? (
          chanel.user ? (
            <div className="flex justify-end items-center space-x-5 text-xs  text-gray-400">
              <Link
                to="/create-video"
                className="hover:text-gray-100 hidden md:block"
              >
                <i className="far fa-plus-square"></i>
              </Link>
              <button className="hover:text-gray-100 hidden md:block">
                <i className="fas fa-th"></i>
              </button>
              <button className="hover:text-gray-100 hidden md:block">
                <i className="fas fa-bell"></i>
              </button>
              <div className="block md:hidden h-full text-normal hover:text-gray-100 text-gray-300">
              ...
              >
                {!chanel.user?.imageProfileUrl ? (
                  <div className="w-6 h-6 rounded-full bg-pink-700  grid place-content-center">
                    <h1 className=" text-gray-50 grid place-content-center">
                      {chanel.user?.username[0]}
                    </h1>
                  </div>
                ) : (
                  <div className="w-6 h-6">
                    <img
                      src={chanel.user?.imageProfileUrl}
                      alt="user profile"
            ...
            <div className="flex justify-end items-center">
              <Link
                to="/sign-in"
                className="text-xs font-thin border border-blue-500 text-blue-500 hover:text-blue-600 rounded-sm px-5 py-1"
              >
                Sign In
              </Link>
            </div>
